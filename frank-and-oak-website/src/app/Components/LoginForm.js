...
const LoginForm = ({ close }) => {
  ...
  const validateform = () =>{
    ...
    if(formData.f_name){
    if(!formData.f_name.trim()) newerror.f_name = "please enter your first name" 
    }

    else{
       newerror.f_name = "please enter your first name" ;
    }

    if(formData.l_name){
        if(!formData.l_name.trim()) newerror.l_name = "please enter your last name" 


    }

    else{
       newerror.l_name = "please enter your last name" ;
    }
    ...
    if(!emailpattern.test(formData.email)) newerror.email = "please enter a valid email" ; 

      const passwordpattern = /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[.@#$%^&+=])[A-Za-z\d.@#$%^&+=]{8,}$/;
     if(!passwordpattern.test(formData.password)) newerror.password = "please enter a valid password" ;
   setErrors(newerror) ;
...
if(!ifvalid) return setTimeout(()=>{setErrors({})},4000)
  ...
  axios.post(`${process.env.NEXT_PUBLIC_URL}/frankandoak/users/generateotp` , formData)
  ...
  const handleregisteruser = (e) =>{
    ...
    axios.post(`${process.env.NEXT_PUBLIC_URL}/frankandoak/users/registerotp` , formData)
    .then((response) =>{
      // console.log(response.data)
      Cookies.set("user_200" , JSON.stringify(response.data));
    })
  ...
  const handleloginuser = (e)=>{
    ...
    console.log(loginform)
        ...
        axios.post(`${process.env.NEXT_PUBLIC_URL}/frankandoak/users/loginuser` , loginform)
    .then((response) =>{
      // console.log(response.data)

      Cookies.set('token' , JSON.stringify(response.data.auth) ,{expires : 7})
    })
          ...
          <li className="w-full grid grid-flow-row place-content-center gap-[15px]">
            ...
            /> */}
              <svg
                ...
                fill="none"
              >
                <g clipPath="url(#clip0_2406_20469)">
                  <path
      ...
      <div className="w-[95%] my-[20px]">
        {signUp ? (
            ...
            <form method="post" className="my-[30px] relative">
              <div className="grid grid-cols-[2fr_2fr] gap-[20px] my-[10px]">
                <div className="relative">
                        {errors.f_name && (<p className="position absolute b-0 text-red-400">{errors.f_name}</p>)}
                  <input
                    type="text"
                    name="f_name"
                ...
                <div className="relative">
                 {errors.l_name && (<p className="position-absolute bottom-0 text-red-400">{errors.l_name}</p>)}
                  <input
                    type="text"
                    name="l_name"
              ...
              <input
                ...
                className="p-[10px] focus:outline-none w-full border position-relative border-black text-[14px] my-[10px]"
                onChange={(e) => { setFormData({ ...formData, email: e.target.value }) }}
                value={formData.email}
              />
                    {errors.email && (<p className="position-absolute bottom-0 text-red-400">{errors.email}</p>)}
              ...
              />
              {errors.password && ((<p className="text-400-red">{errors.password}</p>))}
              <span
          ...
          >
            ...
            <button type="sbumit" className="w-full h-[40px] bg-black text-white" 
