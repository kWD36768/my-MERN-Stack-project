...
function Color() {
  ...
  const handladdcolor = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/adminpanel/colors/addcolors`,
        data
      );

      if (response.status !== 200) {
        alert("colors has not been added successfully");
      }
        let timerInterval;
        Swal.fire({
  ...
  return (
    <div className="w-[90%] bg-white mx-auto rounded-[10px] border my-[150px]">
      ...
      <form action="" method="post" onSubmit={handladdcolor}>
